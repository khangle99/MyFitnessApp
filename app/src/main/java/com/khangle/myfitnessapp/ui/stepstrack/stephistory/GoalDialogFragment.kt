...
class GoalDialogFragment constructor(private val completeion: (Int) -> Unit): BaseDialogFragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_step_goal_dialog, container, false)
        cancelChip = view.findViewById(R.id.cancelStepGoalBtn)
        saveStepGoal = view.findViewById(R.id.saveGoalBtn)
        stepGoalEditText = view.findViewById(R.id.stepGoalEditText)
        return view
    }
    ...
    private fun saveIfnew() {
        ...
        if (goal != this.goal) {
            sharePreferenceUtil.setStepGoal(goal)
            completeion(goal)
        }
        dismiss()
    }
    ...
    private fun loadCurrentGoal() {
        goal = sharePreferenceUtil.getStepGoal()
